...
window.INDUSTRIES = [
  ...
  { key:'fa', en:'FACTORY AUTOMATION', ja:'ファクトリーオートメーション', img:'assets/img/case-logistics.jpg', note:'FA・生産ライン',
  ...
  { key:'plant', en:'PLANT', ja:'プラント', img:'assets/img/case-other.jpg', note:'プラント・化学',
    makers:['moxa','odu'], desc:'プラント・化学設備の堅牢ネットワークと耐環境コネクタ。広域・屋外・防爆エリア近傍での安定通信を実現します。' },
  { key:'measurement', en:'MEASUREMENT', ja:'計測', img:'assets/img/biz-global.jpg', note:'計測・検査',
    makers:['odu','fraba','ipc'], desc:'計測・検査装置向けの高信頼コネクタ・センサ・組込PC。高精度・繰り返し嵌合・長期安定性が求められる用途に対応します。' },
  { key:'other', en:'OTHER', ja:'その他', img:'assets/img/mvimg.jpg', note:'その他業界',
...
/* ===== 営業担当者マスタ（担当別売上）===== */
window.REPS = [
  { name:'鈴木 慶博', dept:'CS営業1課', target:38000000, tel:'03-5825-5331', mobile:'090-1234-5678', email:'suzuki@kmecsone.jp' },
  { name:'田村 直樹', dept:'CS営業1課', target:52000000, tel:'03-5825-5332', mobile:'090-2345-6789', email:'tamura@kmecsone.jp' },
  { name:'佐々木 涼', dept:'CS営業2課', target:20000000, tel:'03-5825-5333', mobile:'090-3456-7890', email:'sasaki@kmecsone.jp' },
];
window.repSales = function(name){
  return window.CUSTOMERS.filter(function(c){return c.rep===name;});
};
window.getRep = (name) => window.REPS.filter(r => r.name === name)[0] || window.REPS[0];
...
window.ADMIN_ORDERS = [
  ...
  { no:'WO-24066', cust:'8006-01', date:'2026/06/23', amount:96000,  route:'Web検索',        status:'returned',  maker:'moxa',  items:'MOXA EDS-518A ×2（初期不良で返品受付）' },
...
/* ===== ニュース / コラム（トップ用）===== */
window.NEWS = [
  { id:'n1', date:'2026.07.02', cat:'新製品情報', title:'【新製品】Moxa TSN-G5016シリーズ — 16ポート TSN対応マネージドイーサネットスイッチ',
    body:'Industry 4.0 の実現に向けて設計された産業用 TSN 対応マネージドスイッチ。12ポートの 10/100/1000BaseT(X) に加え、10GbE SFP+／ギガSFP対応の4スロットを備え、高帯域・低遅延通信を実現します。IEEE 802.1Qbv・IEEE 1588v2 PTP など TSN 関連規格に対応し、動作温度 -40〜70℃・12〜48VDC 冗長デュアル入力・IP40。型番：TSN-G5016-4XGS-SoMD-LV-T ほか。' },
  { id:'n2', date:'2026.06.25', cat:'新製品情報', title:'【新製品】Moxa TWS-3010-SPEシリーズ — SPE/PoDL対応マネージドイーサネットスイッチ',
    body:'現場に信頼性の高い接続を提供する産業用シングルペアイーサネット（SPE）スイッチ。8つの 10Base-T1L（SPE）ポートと2つのギガコンボポートを搭載し、各SPEポートで PoDL 給電（クラス10/11/12）に対応。EtherNet/IP・PROFINET・Modbus TCP、RSTP/STP・MRP 冗長に対応し、DIPスイッチ設定で設置が容易。動作温度 -40〜75℃・5年保証。型番：TWS-3010-SPE-8PB-2GC-T。' },
  { id:'n3', date:'2026.06.23', cat:'イベント情報', img:'assets/img/col4.jpg', title:'【7/23 オンライン】造船メーカー向け「IACS UR E26 の主要要素と Moxa ソリューション」',
    body:'船舶のサイバーセキュリティ規格 IACS UR E26 の主要5要素のうち「識別・防御」に特化し、造船現場の課題と Moxa 製 産業用ネットワーク機器（ファイアウォール・スイッチ・監視プラットフォーム）による解決策を解説するオンラインセミナー。MXview One・EDRシリーズの実機デモも実施。2026/7/23（木）14:00〜14:45・オンライン・参加費無料。登壇：営業部 栗山裕行／技術部FAE 片岡浩一朗。' },
  { id:'n4', date:'2026.06.18', cat:'イベント情報', img:'assets/img/col10.jpg', title:'【7/2 オンライン】アプリケーション別に学ぶ ロータリエンコーダの使い分け（POSITAL）',
    body:'POSITAL のアブソリュート／インクリメンタルエンコーダの使い分けを、アプリケーション別に実例を交えて解説するオンラインセミナー。搬送・建機・再エネ分野などでの選定ポイントを整理します。2026/7/2（木）14:00〜・オンライン開催。' },
  { id:'n5', date:'2026.06.15', cat:'イベント情報', img:'assets/img/col1.jpg', title:'【Moxa 7/9 オンライン】止まりにくく、保守しやすい装置づくりのための通信構成',
    body:'装置内機器の安定接続、装置と工場ネットワークの安全な接続、リモート監視・保守効率化のポイントを、スイッチ・コンバータ・I/O製品の実例を交えて解説。主催：Moxa Japan。2026/7/9（木）11:00〜・オンライン（GoToWebinar）。産業装置メーカーの開発・設計担当、SIer、代理店・商社向け。' },
  { id:'n6', date:'2026.06.04', cat:'イベント情報', img:'assets/img/col3.jpg', title:'【Moxa 6/23 オンライン】IACS UR E26 対応におけるネットワーク設計の課題と未来',
    body:'海事業界のサイバーリスク、「IACS UR E26 とは」、現場課題と Moxa のソリューション、日本海事業界における E26 の未来を初心者向けに解説。主催：Moxa Japan（当社は正規代理店）。2026/6/23（火）11:00〜・オンライン（GoToWebinar）。船主・オペレーター／造船所・設計会社／SIer 向け。' },
  { id:'n7', date:'2026.05.21', cat:'イベント情報', img:'assets/img/col2.jpg', title:'【6/11 オンライン】基板検査治具の段取り替えをもっと手軽に',
    body:'新製品の基板検査治具について、段取り替え（品種切替）をより手軽にする内容を紹介するオンラインセミナー。2026/6/11（木）14:00〜・オンライン開催。' },
  { id:'n8', date:'2026.05.18', cat:'お知らせ', title:'Moxa、産業用無線機器 AWK（Wi-Fi5/Wi-Fi6）シリーズで JC-STAR 認定を取得',
    body:'当社が正規代理店を務める Moxa が、IEC 62443 等の制御システムセキュリティ規格に基づくセキュアバイデザイン設計（AWK firmware V2.X以降）の産業用無線機（Wi-Fi）について、JC-STAR の適合ラベルを取得しました。対象は Wi-Fi5 対応の AWK-3252A／4252A／1151C、Wi-Fi6 対応の AWK-3262A／4262A／1161A／1161C／1165A／1165C など。' },
  { id:'n9', date:'2026.05.12', cat:'イベント情報', img:'assets/img/col7.jpg', title:'【製造業向け】EU サイバーレジリエンス法（CRA）最新動向 解説セミナー',
    body:'OTネットワークのセキュリティ対策が注目される中、欧州で施行された CRA 法への対策を扱うスペシャルセミナー。CRA 法の概要解説、対策に役立つ Moxa の産業用ネットワークデバイスの紹介に加え、三菱電機株式会社をゲストに迎え、IEC 62443-4-1 認証取得支援での経験や現場での OT セキュリティ対策状況を紹介します。' },
  { id:'n10', date:'2026.04.28', cat:'イベント情報', img:'assets/img/col9.jpg', title:'ワイヤレスジャパン ×ワイヤレス・テクノロジー・パーク（WTP）2026 に出展いたします',
    body:'同展示会に出展し、導入しやすく冗長性を実現する産業用無線通信機器など Moxa のソリューションを展示します。会期：2026/5/27（水）〜29（金）10:00〜18:00（最終日17:00）、会場：東京ビッグサイト 西3・4ホール、ブース W-58。5/28（木）13:30〜 出展社セミナーも実施。招待券をご希望の方はお問い合わせください。' },
];
...
/* ===== セミナー・ウェビナー（event-detail / event-apply で共有） ===== */
window.SEMINARS = {
    'iacs-e26': {
      tag:'造船メーカー向け', mode:'オンライン', tool:'Microsoft Teams', fee:'無料', img:'assets/img/case-ship.jpg',
      date:'2026.07.23', dow:'木', time:'14:00〜14:45', span:'45分',
      title:'造船メーカー向け IACS UR E26 の主要要素と Moxa ソリューション',
      sub:'船舶のデジタル化における新基準「IACS UR E26」',
      lead:'2024年施行の IACS UR E26 は、船舶のサイバーセキュリティに関する新しい統一規則です。本ウェビナーでは、E26 が定義する5つの主要要素のうち「識別」「防御」にフォーカスし、造船現場の課題と、Moxa 製 産業用ネットワーク機器による解決策を実機デモを交えて解説します。',
      target:['造船メーカーをはじめとした IACS UR E26 対象者の方','船上ネットワークの設計・保守に携わる方','セキュアな船舶ネットワーク構築を検討中の方'],
      goals:['IACS UR E26 の5つの主要要素のうち「識別」「防御」の要点が分かる','規格適合への具体的な道筋が理解できる','「止まらない・守れる」船舶ネットワークの構築手法が学べる'],
      program:[
        {t:'IACS UR E26「識別・防御」の現場課題', d:'海事業界のサイバーリスクと、E26 が求める「識別・防御」の要件を、造船現場の視点で解説します。'},
        {t:'Moxa ソリューションの実機デモ', d:'ネットワーク可視化「MXview One」と、産業用セキュアルータ／ファイアウォール「EDRシリーズ」の実機デモンストレーションをご覧いただきます。'}
      ],
      solutions:[
        {n:'MXview One', d:'産業用ネットワークの可視化・監視プラットフォーム。機器・トポロジを把握し、異常を早期に検知します。'},
        {n:'EDRシリーズ', d:'産業用セキュアルータ／ファイアウォール。OT/IT の分離・VPN・侵入防御でネットワークを守ります。'}
      ],
      speakers:[
        {name:'栗山 裕行', org:'株式会社ケーメックスONE 営業部', role:'IACS UR E26「識別・防御」について解説'},
        {name:'片岡 浩一朗', org:'株式会社ケーメックスONE 技術部 FAEグループ', role:'MXview One・EDRシリーズの実機デモンストレーション'}
      ],
      host:'株式会社ケーメックスONE',
      faq:[
        {q:'参加費はかかりますか？', a:'無料でご参加いただけます。'},
        {q:'当日参加できない場合、資料はもらえますか？', a:'ご登録いただいた方には、セミナー終了後に講義資料をメールでお送りします。'},
        {q:'視聴に必要なものは？', a:'インターネット環境と、Microsoft Teams を視聴できる端末（PC 推奨）をご用意ください。登録後に視聴用URLをメールでお送りします。'},
        {q:'個別に相談することはできますか？', a:'はい。セミナー後の個別相談も承っております。お問い合わせください。'},
        {q:'同業者でも参加できますか？', a:'恐れ入りますが、同業者様のお申し込みはお断りする場合がございます。'},
        {q:'途中からの参加や退出は可能ですか？', a:'可能です。ご都合の良い範囲でご視聴ください。'}
      ]
    },
    'moxa-device': {
      tag:'装置メーカー向け', mode:'オンライン', tool:'GoToWebinar', fee:'無料', img:'assets/img/moxa-switch.jpg',
      date:'2026.07.09', dow:'木', time:'11:00〜', span:'—', title:'止まりにくく、保守しやすい装置づくりのための通信構成',
      sub:'スイッチ・コンバータ・I/O製品を活用した実例紹介',
      lead:'装置内の機器同士を安定してつなぐ方法、装置と工場ネットワークを安全につなぐ方法、リモート監視・保守を効率化するポイントを、実際の導入事例を交えて解説します。主催：Moxa Japan。',
      target:['産業装置メーカーの開発・設計担当者','装置立ち上げ工数・保守コストに課題を持つ部門','スマートファクトリー化を支援する SIer','提案力を高めたい代理店・商社'],
      goals:['装置内機器を安定してつなぐ通信構成が分かる','装置と工場ネットワークを安全につなぐ方法が分かる','リモート監視・保守で効率化するポイントが分かる'],
      program:[{t:'装置内接続の実例', d:'スイッチ・コンバータ・I/O製品を活用した安定接続の実例を紹介します。'},{t:'工場ネットワークとの安全な接続', d:'OT/IT 境界の考え方と、安全に外部とつなぐ構成を解説します。'},{t:'リモート監視・保守', d:'保守効率を高める監視・遠隔対応のポイントを紹介します。'}],
      solutions:[], speakers:[], host:'Moxa Japan 合同会社（当社は正規代理店）',
      faq:[{q:'参加費はかかりますか？', a:'無料でご参加いただけます。'},{q:'資料はもらえますか？', a:'ご登録いただいた方に、後日資料をご案内する場合があります。'}]
    },
    'moxa-e26design': {
      tag:'海事業界向け', mode:'オンライン', tool:'GoToWebinar', fee:'無料', img:'assets/img/case-ship.jpg',
      date:'2026.06.23', dow:'火', time:'11:00〜', span:'—', title:'IACS UR E26 対応におけるネットワーク設計の課題と未来',
      sub:'セキュアな船上ネットワーク構築の要件を初心者向けに',
      lead:'海事業界におけるサイバーリスク、「IACS UR E26 とは」、現場の課題と Moxa のソリューション、日本海事業界における E26 の未来を、初心者の方にも分かりやすく解説します。主催：Moxa Japan。',
      target:['船主・オペレーター','造船所・設計会社','コンサルタント・システムインテグレーター'],
      goals:['IACS UR E26/27 の概要が分かる','セキュアな船上ネットワーク設計の要件が理解できる','現場課題への Moxa のアプローチが分かる'],
      program:[{t:'海事業界のサイバーリスク', d:'船舶を取り巻くサイバーセキュリティの動向を整理します。'},{t:'IACS UR E26 とは', d:'2024年施行の規則の要点を初心者向けに解説します。'},{t:'現場課題と Moxa ソリューション', d:'設計上の課題と対応策を紹介します。'}],
      solutions:[], speakers:[], host:'Moxa Japan 合同会社（当社は正規代理店）',
      faq:[{q:'初心者でも大丈夫ですか？', a:'はい。初めての方にも分かりやすい内容です。'},{q:'参加費は？', a:'無料です。'}]
    },
    'encoder': {
      tag:'POSITAL', mode:'オンライン', tool:'—', fee:'無料', img:'assets/img/cat-measure.jpg',
      date:'2026.07.02', dow:'木', time:'14:00〜', span:'—', title:'アプリケーション別に学ぶ ロータリエンコーダの使い分け',
      sub:'POSITAL アブソリュート／インクリメンタルエンコーダ 実例紹介',
      lead:'POSITAL のアブソリュートエンコーダとインクリメンタルエンコーダの使い分けを、アプリケーション別に実例を交えて解説します。搬送・建機・再生可能エネルギー分野などでの選定ポイントを整理します。',
      target:['エンコーダの選定に迷っている方','搬送・建機・再エネ分野の設計担当者','モーション制御・位置決めに携わる方'],
      goals:['アブソリュート／インクリメンタルの違いと使い分けが分かる','アプリケーション別の選定ポイントが分かる'],
      program:[{t:'エンコーダの基礎', d:'方式ごとの特性を整理します。'},{t:'アプリ別の使い分け', d:'実例を交えて選定の考え方を解説します。'}],
      solutions:[], speakers:[], host:'株式会社ケーメックスONE',
      faq:[{q:'参加費は？', a:'無料です。'}]
    },
    'cra': {
      tag:'製造業向け', mode:'オンライン', tool:'—', fee:'無料', img:'assets/img/moxa-router.jpg',
      date:'2026.05.12', dow:'', time:'—', span:'—', title:'EU サイバーレジリエンス法（CRA）最新動向 解説セミナー',
      sub:'OTセキュリティ対策と Moxa 製品の活用',
      lead:'OTネットワークのサイバーセキュリティ対策が注目される中、欧州で施行された CRA 法への対策を扱うスペシャルセミナー。CRA の概要解説、対策に役立つ Moxa の産業用ネットワークデバイスの紹介に加え、三菱電機をゲストに迎え IEC 62443-4-1 認証取得支援での経験や現場の OT セキュリティ対策状況を紹介します。',
      target:['CRA 対策が必要な装置メーカーの設計・担当者','関連商材を探す商社担当者'],
      goals:['CRA 法の概要と影響が分かる','対策に役立つ産業用ネットワークデバイスが分かる','IEC 62443-4-1 認証取得支援の実際が分かる'],
      program:[{t:'CRA 法の概要', d:'欧州 CRA 法の要点と製造業への影響を解説します。'},{t:'Moxa 製品による対策', d:'対策に役立つデバイスを紹介します。'},{t:'ゲスト講演（三菱電機）', d:'IEC 62443-4-1 認証取得支援の経験・現場の OT セキュリティ対策を紹介します。'}],
      solutions:[], speakers:[{name:'ゲスト：三菱電機株式会社', org:'', role:'IEC 62443-4-1 認証取得支援の経験を紹介'}], host:'株式会社ケーメックスONE',
      faq:[{q:'参加費は？', a:'無料です。'}]
    }
  };
window.getSeminar = (id) => window.SEMINARS[id] || window.SEMINARS['iacs-e26'];
